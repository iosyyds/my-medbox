...
function loadScript() {
  return new Promise((resolve, reject) => {
    ...
    script.onerror = () => reject(new Error('Tesseract.js 脚本加载失败，请检查网络'));
...
async function initWorker(Tesseract) {
  ...
  try {
    // v5 简化用法：createWorker(langs, oem, options)
...
export async function recognizeText(imageData, onProgress) {
  try {
    ...
    // v5 recognize 返回 { data: { text, confidence, ... } }
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        // 放大图片以提高识别率（小图放大到最短边800px）
        const minDim = Math.min(img.width, img.height);
        const scale = minDim < 800 ? Math.max(1, 800 / minDim) : 1;
 ...
 * 3. 匹配药品知识库
 * @param {string} imageData - base64图片
 * @param {function} onProgress - 进度回调 (0-1, status)
...
export async function recognizeMedicine(imageData, onProgress) {
  ...
  const text = await recognizeText(processedImage, (p, s) => {
    // recognizeText 内部进度 0-1 映射到总进度 0.1-0.85
