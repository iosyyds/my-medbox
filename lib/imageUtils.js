...
export function computeImageHash(imageDataUrl) {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => reject(new Error('图片特征计算失败'));
...
// 识图搜索
export function searchByImage(targetFeature, medicines, threshold = 0.4) {
  if (!targetFeature || !Array.isArray(medicines)) return [];
