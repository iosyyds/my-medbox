...
export default function Home() {
  ...
  const openEditModal = (med) => {
    ...
    setFormData({
      ...
      imageFeature: med.imageFeature || null,
    });
  ...
  const handleSaveMed = () => {
    ...
    const med = {
      ...
      imageFeature: formData.imageFeature,
  ...
  const handleCameraSearch = async (e) => {
    ...
    try {
      ...
      if (results.length === 0) {
        showToast('未找到相似药品，可先添加记录', 'error');
      }
    } catch (err) {
  ...
  return (
    <main>
          ...
          <div className="header-top">
            ...
            <div className="stats">
              <div className="stat-item">
                ...
                <div className="stat-label">药品</div>
              </div>
              <div className="stat-item">
                <div className="stat-num" style={{ color: '#fecaca' }}>
                  {expiredCount}
                </div>
      ...
      <div className="container">
        ...
        {expiredCount + expiringCount > 0 && (
          <div className="expire-banner">
            <WarningIcon size={22} />
            <div className="expire-banner-text">
              有 <strong>{expiredCount + expiringCount}</strong> 种药品已过期或即将过期，请及时清理
            </div>
          </div>
        )}
        ...
        <div className="section-title">
          <h2>我的药品</h2>
          <span>共 {filteredMedicines.length} 种</span>
        </div>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                {recognizing && (
                  <div className="recognize-progress">
                    ...
                    <div className="recognize-progress-bar">
                      <div
                        className="recognize-progress-fill"
                ...
                {recognizeError && !recognizing && (
                  <div className="recognize-error">
                    <div className="recognize-error-header">
                      <WarningIcon size={14} />
                      <span>识别失败</span>
                    </div>
                    <p className="recognize-error-text">{recognizeError}</p>
                    {formData.image && (
                      <button className="btn-rerecognize" onClick={handleReRecognize}>
                        <ScanIcon size={14} />
                        重新识别
                      </button>
                    )}
                ...
                {recognizedText && !recognizing && (
                  <div className="recognized-text">
                    <div className="recognized-text-header">
                      ...
                      <span>识别到的文字</span>
                      {formData.image && (
                        <button className="btn-rerecognize-small" onClick={handleReRecognize}>
                          <ScanIcon size={12} />
                          重新识别
                        </button>
                      )}
              ...
              <div className="form-group">
                <label className="form-label">
                  药品名称 <span className="required">*</span>
                </label>
                <input
                  type="text"
              ...
              <div className="form-group">
                ...
                <textarea
                  ...
                  placeholder="如：用于缓解轻至中度疼痛，如头痛、关节痛、偏头痛、牙痛、肌肉痛、神经痛、痛经；也用于普通感冒或流行性感冒引起的发热"
              ...
              <div className="form-group">
                ...
                <textarea
                  className="form-textarea"
                  placeholder="如：口服。成人一次1粒，一日2次（早晚各一次）"
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input
                    type="text"
                    className="form-input"
                    placeholder="如：孕妇、哺乳期妇女禁用"
                    value={formData.taboo}
                    onChange={(e) => setFormData((prev) => ({ ...prev, taboo: e.target.value }))}
                  />
                </div>
                <div className="form-group">
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="detail-img">
                {detailMed.image ? (
                  <img src={detailMed.image} alt={detailMed.name} />
                ) : (
                  <div className="placeholder">
                    <PillIcon size={64} />
                  </div>
              ...
              <div className="detail-sub">
                <span>{getTypeLabel(detailMed.type)}</span>
                <span>·</span>
                <span className={`status-${getExpireStatus(detailMed.expireDate).status}`}>
                  {getExpireStatus(detailMed.expireDate).label}
                  {getExpireStatus(detailMed.expireDate).days !== null &&
                    `（${getExpireStatus(detailMed.expireDate).days >= 0 ? '还剩' : '已过'} ${Math.abs(
                      getExpireStatus(detailMed.expireDate).days
                    )} 天）`}
                </span>
              </div>
            ...
            <div className="modal-footer">
              <button className="btn btn-secondary" onClick={() => openEditModal(detailMed)}>
                <EditIcon size={17} />
                编辑
              </button>
              <button className="btn btn-danger" onClick={() => handleDeleteMed(detailMed)}>
                <TrashIcon size={17} />
                删除
              </button>
            </div>
            ...
            <div className="modal-body">
              <div className="form-group">
                ...
                <input
                  type="text"
                  className="form-input"
                  placeholder="如：布洛芬 / 头痛发烧 / 胃不舒服"
                  value={aiInput}
                  onChange={(e) => setAiInput(e.target.value)}
                  onKeyDown={(e) => e.key === 'Enter' && handleAiGenerate()}
                />
              </div>
              <button className="btn btn-primary btn-block" onClick={handleAiGenerate} style={{ marginBottom: 16 }}>
                  ...
                  {aiResults.map((kw, i) => (
                    <div key={i} className="keyword-item" onClick={() => useKeyword(kw.main)}>
                      ...
                      <div className="kw-text">
                        <div className="kw-main">{kw.main}</div>
                        <div className="kw-sub">{kw.sub}</div>
                      </div>
      ...
      {showCameraModal && (
        <div
          className="modal-overlay show"
          onClick={(e) => e.target === e.currentTarget && setShowCameraModal(false)}
        >
          <div className="modal">
            <div className="modal-header">
              <h3>图片搜药</h3>
              ...
              {cameraResults.length > 0 && (
                <div style={{ marginTop: 20 }}>
                  <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 12 }}>
                    找到 {cameraResults.length} 个相似药品：
                  </p>
                  {cameraResults.map((result, i) => (
                    <div
                      key={i}
                      className="search-result-item"
                      onClick={() => viewFromCameraResult(result.medicine)}
                    >
                      <div className="search-result-img">
                        {result.medicine.image ? (
                          <img src={result.medicine.image} alt={result.medicine.name} />
                        ) : (
